 ...
 */
fun Int.weatherCodeToCondition(dayNightCode: Int?): WeatherCondition = when (this) {
    0 -> {
        if (dayNightCode != null)
            if (dayNightCode.isDay()) WeatherCondition.CLEAR else WeatherCondition.STARRY
        else WeatherCondition.CLEAR
    }

    1 -> {
        if (dayNightCode != null)
            if (dayNightCode.isDay()) WeatherCondition.MOSTLY_CLEAR else WeatherCondition.STARRY
        else WeatherCondition.MOSTLY_CLEAR
    }

    2 -> {
        if (dayNightCode != null)
            if (dayNightCode.isDay()) WeatherCondition.PARTLY_CLOUDY else WeatherCondition.PARTLY_STARRY
...
fun Int.codeToDayAndNight(): DayNight =
    if (this == 1) DayNight.DAY else DayNight.NIGHT


fun Int.weatherCodeToWeatherIcon(): WeatherIcon = when (this) {
